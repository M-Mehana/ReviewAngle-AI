...
## Arabic-first output and compatibility

New UI/API output locales are ar, ar-EG and ar-SA. The default is neutral Arabic with RTL results, including dialogs when the interface is English. Source language detection remains en/ar/mixed/und; ingestion and original evidence are independent of output locale. StoredOutputLanguage retains legacy en for reads/exports; new creation and paid regeneration reject it.

## Deterministic creative portfolio

The portfolio planner runs before angle prose generation. Each slot fixes its underlying supported motivation, primaryThemeId, allowed themeIds, creativeDevice and evidenceMode. New slots normally use implicit_evidence. Evidence from a review supports the claim internally; it does not require displaying or reading the review in the ad. The compact device vocabulary covers demo, routine, close-up, lifestyle, comparison, objection/answer, education, text, POV, social proof, product feature, problem/solution and talking head.

For saved sets, independent review support determines reservation order. The strongest valid social-proof execution is reserved; additional quote-led executions receive unused non-social devices while keeping their valid evidence/motivation. Passing existing candidates are preserved before repairs. The actual opening/UGC presentation determines the creative signature; renaming review interpretation or reading comments as objection-answer cannot bypass the one-explicit-social-proof cap. Underlying theme concepts identify motivations independently of review framing.

New/repaired ad hooks, copy, CTA, opening and UGC must obey the assigned slot and implicit/explicit evidence mode. Internal rationale and original evidence storage/UI remain separate. Saved valid prose is preserved and checked for current execution/set integrity. Existing exact quote, scope, target, unsupported-claim, creator-provenance, localization and deterministic score checks continue to apply. Unknown execution families cannot satisfy an assigned generation/repair slot.

A rejected slot receives one QUALITY repair and optionally one FALLBACK repair. The model receives its fixed slot, selected evidence, rejection reasons and occupied creative families, and cannot select another device itself. Failed results are omitted rather than filling a ten-angle target. Validation is deterministic and uses no embedding/judging API; manual semantic review remains required because lexical patterns are not a general correctness guarantee.

Current staging routing is FAST=gpt-5.6-luna, QUALITY=gpt-5.6-luna and optional FALLBACK=gpt-5.6-terra. FAST handles extract/group; QUALITY handles intelligence, angles, hooks, UGC and translation. Blank tier variables retain OPENAI_MODEL compatibility; blank FALLBACK disables escalation. Schemas/evidence fields and score v1 remain unchanged. Private checkpoints and per-stage usage reports stay ignored.
